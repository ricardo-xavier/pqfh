...
public class Debug {
	...
	public static void gravaTela(Terminal terminal, int n) {
		...
		synchronized (terminal.getLog()) {
			...
			terminal.getLog().printf("%s %d,%d%n", Thread.currentThread().getName(), terminal.getLin(),
					terminal.getCol());
			terminal.getLog().println(n + " " + terminal.getR());

			for (int y = 0; y < Terminal.getLinhas(); y++) {
				terminal.getLog().printf("%02d %s%n", y + 1,
						new String(terminal.getDados()[y], 0, Terminal.getColunas()));
			}
			terminal.getLog().println();
