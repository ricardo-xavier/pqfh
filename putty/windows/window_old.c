      ...
      case WM_SYSKEYUP:
         ...
         */
        {
            ...
            if (wParam == VK_PROCESSKEY || /* IME PROCESS key */
                ...
                /* ricardo */
                wParam = ava_converte_decimal(wParam, term->curs.y, term->curs.x);
...
/*
                if ((message == WM_KEYDOWN) 
                       ...
                       && (term->curs.x > 0)
                       && (term->curs.y < 25)
                       && (linhas[term->curs.y][term->curs.x-1] >= '0')
                       && (linhas[term->curs.y][term->curs.x-1] <= '9')
                       && ((wParam == VK_DECIMAL) || (wParam == 194))) {
if (1) {
int i;
FILE *f=fopen("putty.log", "a");
fprintf(f, "%d %d %d\n", term->curs.y, term->curs.x, linhas[term->curs.y][term->curs.x-1]);
for (i=0; i<25; i++) {
fprintf(f, "%s\n", linhas[i]);
}
fclose(f);
}
                    len = TranslateKey(message, VK_OEM_COMMA, lParam, buf);
                } else 
                ...
                if ((message == WM_KEYDOWN) 
                    ...
                    if ((wParam == VK_OEM_PERIOD) 
                           || (wParam == VK_OEM_COMMA)
                           || (wParam == VK_DECIMAL)
                           || (wParam == 194)) { // . do teclado numerico
if (1) {
int i;
FILE *f=fopen("putty.log", "a");
fprintf(f, "%d %d %d\n", term->curs.y, term->curs.x, linhas[term->curs.y][term->curs.x-1]);
...
fclose(f);
                        ...
                        len = TranslateKey(message, 
                              wParam == VK_OEM_PERIOD ? VK_OEM_COMMA 
                                        : VK_OEM_PERIOD, lParam, buf);
                    }
                }
*/
/*
                if ((wParam >= 96) && (wParam <= 105)) {
                    last_key = '0' + (wParam - 96);
                } else {
                    last_key = (char) wParam;
                }
*/
